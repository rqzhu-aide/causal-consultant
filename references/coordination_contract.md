...
`foundation_gate.status` never stores user-forced progress. User-directed continuation belongs under `main_skill.user_directed` and `analysis.route_commitment_status`. Gate-not-ready exploratory/progress analysis may produce code, diagnostics, and source-plus-HTML artifacts, but it keeps the relevant gatekeeper fields non-handoff and claim strength bounded until the route and production materials genuinely become ready. If actual modeling runs before foundation readiness, use `analysis.production_loop`, `analysis.activated_method_job_subskills`, and compact `subskill_analyses` records only as an execution/audit ledger; do not treat those records as production-gate eligibility.
...
The same ledger can also be used for explicitly user-directed, gate-not-ready exploratory execution before foundation readiness. In that exception, `project.current_phase` remains `foundation` until delivery, `analysis.route_commitment_status` remains `user-directed`, `foundation_gate.can_support_causal_commitment` remains false, and `production_gate.can_handoff_to_report_writer` remains false.
