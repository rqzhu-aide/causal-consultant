...
Runtime contract: follow `references/design_execution_contract.md` using design id `difference_in_differences`. Keep any named support route inside this DiD design scope.
...
In approved deep execution, append one compact `artifact_records` entry according to `references/design_execution_contract.md`. Include DiD specifics in the entry summary or in a note/manifest inside the output location, such as:
