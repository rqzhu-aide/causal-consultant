...
Runtime contract: follow `references/design_execution_contract.md` using design id `randomized_assignment`. Keep any named support route inside this randomized-design scope.
...
In approved deep execution, append one compact `artifact_records` entry according to `references/design_execution_contract.md`. Include randomized-assignment specifics in the entry summary or in a note/manifest inside the output location, such as:
