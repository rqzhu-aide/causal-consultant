...
Runtime contract: follow `references/design_execution_contract.md` using design id `regression_discontinuity`. Keep any named support route inside this RD design scope.
...
In approved deep execution, append one compact `artifact_records` entry according to `references/design_execution_contract.md`. Include RD specifics in the entry summary or in a note/manifest inside the output location, such as:
