...
Runtime contract: follow `references/design_execution_contract.md` using design id `single_time_observational`. Keep any named support route inside this observational-design scope.
...
In approved deep execution, append one compact `artifact_records` entry according to `references/design_execution_contract.md`. Include observational-design specifics in the entry summary or in a note/manifest inside the output location, such as:
