...
- `method_lead`: causal question, candidate/selected framework, estimand set, validity requirements, DAG/theory, diagnostics, sensitivity, wording boundary.
- `analysis_state`: recommended/activated subskills, active sidecars, limitations, report draft path.
- existing `subskill_records` for related activated modules.

Do not duplicate reviewer-owned fields. Return compact feedback that those reviewers can use.

## Standard Output

Use `assets/method_job_subskill_record_template.yaml` when a durable record is needed.

Every activated method/task subskill should return:

- what it was asked to decide;
- module type and role;
- inputs reviewed and provenance level;
- fit summary: direct, adapted, exploratory, blocked, or not applicable;
- assumptions, diagnostics, and sensitivity needs;
- limitations and blockers;
- bounded requests for `data_analyst`, `method_lead`, `domain_expert`, user, or another specialist;
- report support bullets;
...
- `method_lead` decides causal validity, target fit, and claim boundaries after data work.
