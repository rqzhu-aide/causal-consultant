...
2. Let reviewers update their own sections and their assigned `variable_roster` fields through the workflow in `backend_workflow.md`; refresh `data_analyst.analysis_alignment` when data, claims, framework, diagnostics, report target, or earlier warnings make it relevant.
