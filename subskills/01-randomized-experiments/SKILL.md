...
| Complication detected | Keep `01`? | Additional subskill |
|---|---:|---|
| Noncompliance, crossover, treatment switching | yes | `09-instrumental-variables` for CACE/LATE; `16` for adherence/missingness if relevant |
| Time-to-event outcome, censoring, competing risks | yes | `11-survival-competing-risks` |
| Missing outcomes, attrition, post-randomization selection | yes | `16-missingness-measurement-selection` |
| Spillovers, household/network/marketplace interference | yes, but only for design audit | `13-interference-spillovers` |
| Heterogeneous effects, uplift, treatment rules | yes | `05-heterogeneous-effects-policy` |
| Dynamic regimes, repeated randomized decisions, SMART | yes | `06-longitudinal-gmethods` |
| Mediation, mechanisms, post-treatment pathways | yes | `12-mediation` |
| Fuzzy RD with randomization near threshold | maybe | `08-regression-discontinuity` and `09-instrumental-variables` |
| Not actually randomized | no as primary | route back to root: `02`, `07`, `08`, `09`, `10`, or descriptive analysis |
...
2. **Assignment variable is unavailable.** If the dataset contains only treatment received/exposure, but no randomized assignment, the ITT estimand cannot be directly estimated. Route to `02` or `09` depending on whether assignment can serve as an instrument.
3. **Dataset conditions on post-randomization events.** If the dataset includes only exposed, triggered, compliant, retained, surviving, or logged units, warn that the all-randomized ITT estimand may be lost. Activate `16` and possibly redefine the estimand.
4. **Treatment received is the target despite noncompliance.** If the user wants effect of actual treatment receipt, activate `09-instrumental-variables` for CACE/LATE or treat as observational if IV assumptions fail.
5. **Interference is central.** If one unit's treatment can affect another unit's outcome, activate `13` and redefine exposure mappings.
6. **Outcome is survival/competing risk.** Use `11` for estimand and estimator while preserving `01` for assignment audit.
7. **Policy/time-series structure dominates.** If the user describes a policy change, rollout, or adoption without individual randomization, route to `07` or `10`.
...
under IV assumptions: relevance, independence from potential outcomes, exclusion restriction, and monotonicity for LATE interpretation. Route to `subskills/09-instrumental-variables/`.
...
- Use `09-instrumental-variables` when noncompliance/crossover makes treatment receipt differ from randomized assignment.
- Use `11-survival-competing-risks` for time-to-event endpoints, censoring, competing events, RMST, and cumulative incidence.
- Use `16-missingness-measurement-selection` for attrition, missing outcomes, post-randomization exclusions, triggered-only datasets, or logging failures.
- Use `13-interference-spillovers` for spillovers, contamination, marketplaces, networks, classrooms, households, or cluster exposure mappings.
- Use `05-heterogeneous-effects-policy` for subgroup/CATE/uplift/policy-learning analyses.
- Use `06-longitudinal-gmethods` for SMART, dynamic regimes, repeated treatments, or crossover analyses with time-varying treatment.
- Use `12-mediation` for direct/indirect effects and mechanisms.
