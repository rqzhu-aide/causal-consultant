...
- self-selected treatment: `02-point-treatment-observational`;
- policy timing: `07-did-event-study`;
- cutoff eligibility: `08-regression-discontinuity`;
- instrument/encouragement without full compliance: `09-instrumental-variables`;
- aggregate treated time series: `10-synthetic-control-time-series`.
...
- route to `09-instrumental-variables`;
...
- route to `16-missingness-measurement-selection`;
...
Route to `11-survival-competing-risks`. Define whether the target is:
...
| Treatment receipt differs from assignment | Keep ITT in `01`; use `09` for CACE/LATE. |
| Only triggered/exposed users are in dataset | Warn about post-randomization selection; use `16`; define triggered estimand if scientifically intended. |
| Outcome is time-to-event | Use `11`; keep `01` for randomization audit. |
| Interference/spillovers plausible | Use `13`; redefine exposure mapping. |
| CATE/uplift requested | Use `05`; require honest or held-out evaluation. |
| Dynamic regimes/SMART | Use `06`; define regime-level estimands. |
| Mechanism/mediator requested | Use `12`; do not adjust for mediator in primary total-effect analysis. |
