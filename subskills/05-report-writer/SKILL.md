...
- During `causal_specification`, turn the notebook into a report skeleton: causal claim(s), estimand set, framework, DAG/theory, assumptions, data feasibility, diagnostics plan, wording boundary, and unresolved tensions.
...
- if `bounded_continuation.requested` and `bounded_continuation.acknowledged_limits` are true, the report must stay within `bounded_continuation.allowed_scope` and avoid `bounded_continuation.prohibited_claims`.
...
- `method_lead`: causal question variants, estimand set, selected framework, validity requirements, DAG/theory, assumptions, method-literature guidance, methods/subskills, diagnostics, sensitivity, and wording boundary.
...
- `analysis_state` and `artifact_index`: existing outputs, report assets, report working draft path, discovery sidecar material, and limitations.
...
Start the Markdown project notebook/working report during `project_exploration` once there is durable content to preserve. Do not wait for `causal_specification` if the user's interests, domain context, early analysis ideas, or data reality are already taking shape. Update it after each meaningful user-priority update, reviewer update, sidecar output, artifact creation, diagnostic result, or report-production change. Use `assets/working_report_template.md` as the default structure when starting a new draft.
...
- report bookkeeping: paper-like discussion notes, decisions, tensions, references, artifacts, active modules, and open questions;
...
Before compiling an artifact, make sure the report lane and claim limits are clear. Put full reports, Markdown drafts, notebooks, rendered HTML, slides, captions, appendices, tables, figures, and code artifacts in artifact folders and register durable paths in `artifact_index`. Keep YAML updates compact:
