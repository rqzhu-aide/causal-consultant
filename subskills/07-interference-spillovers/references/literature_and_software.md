...
- Rubin (1980) and later potential-outcome work formalized no-interference/SUTVA as a standard assumption; this module is relevant when that assumption is doubtful.
...
# Replace edge columns, unit id, treatment, outcome, and clusters.
library(igraph)
library(geepack)

g <- graph_from_data_frame(edges, directed = FALSE, vertices = units)
analysis_data$neighbor_A <- vapply(V(g)$name, function(i) {
  mean(analysis_data$A[neighbors(g, i)$name], na.rm = TRUE)
}, numeric(1))
gee_fit <- geeglm(Y ~ A + neighbor_A + X1 + X2,
