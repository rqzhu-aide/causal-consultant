...
- Need weak comparison group or aggregate units: consider `synthdid` and coordinate with `13-synthetic-control-time-series`.
- Need conditional parallel trends with many covariates: coordinate with `31`/`32`; use DR-DiD or DoubleML DiD if the estimator matches the target.
...
- Need continuous treatment or intensity: coordinate with `23-dose-response-effects`; do not shoehorn into binary adoption without justification.
