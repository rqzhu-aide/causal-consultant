...
| Few treated units or aggregate policy | Synthetic control, synthetic DiD, randomization/placebo inference | Better suited for aggregate intervention settings | May belong partly to `13-synthetic-control-time-series` |
| Continuous or dose treatment | Continuous DiD or dose-response module | Binary adoption DiD is not enough | Activate `23-dose-response-effects` |
| Spillovers or contamination | Spillover/interference design | Controls may be indirectly treated | Activate `14-interference-spillovers` |
...
- `13-synthetic-control-time-series`: one/few treated units, donor pools, synthetic DiD, comparative interrupted time series.
- `14-interference-spillovers`: treatment contamination, network or spatial spillovers, control exposure.
- `20-heterogeneous-effects`: subgroup, cohort-specific, or effect-modification targets beyond standard group-time heterogeneity.
- `23-dose-response-effects`: continuous or intensity treatment versions of DiD.
- `24-transportability-generalizability`: when DiD estimates must be transported to another target population.
- `30-matching-weighting-balance`: pre-treatment matching/weighting or composition support.
- `31-doubly-robust-estimation`: DR-DiD and influence-function style reporting.
- `32-double-machine-learning`: flexible nuisance models under conditional parallel trends.
