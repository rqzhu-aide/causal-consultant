...
| [`rdhte`](https://arxiv.org/abs/2507.01128) | R, Stata | Heterogeneous treatment effects in sharp RD | Emerging support for RD heterogeneity | Newer ecosystem; coordinate with `20-heterogeneous-effects` |
...
| [`ivreg`](https://zeileis.github.io/ivreg/) | R | Fuzzy RD as local IV/2SLS sensitivity | Good diagnostics and IV reporting | Must restrict to local window and coordinate with RD diagnostics |
...
- Need fuzzy compliance or encouragement at a cutoff: use `rdrobust` fuzzy support and activate `12-instrumental-variables`.
...
- Need a date cutoff: treat it cautiously; coordinate with time-series, DiD, or synthetic-control modules if trends/shocks could drive the discontinuity.
- Need broad generalization: coordinate with `24-transportability-generalizability`; RD alone is local.
