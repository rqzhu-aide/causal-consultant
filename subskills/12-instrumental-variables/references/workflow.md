...
- `07-randomized-assignment-and-experiments`: encouragement designs, ITT, assignment integrity, noncompliance.
- `11-regression-discontinuity`: fuzzy RD and local Wald at cutoff.
- `09-longitudinal-gmethods`: time-varying exposure/outcome histories with instruments.
- `22-mediation`: IV mediation only under specialized assumptions; keep pathway claims careful.
- `31-doubly-robust-estimation`: influence-function/DR IV variants if implemented.
- `32-double-machine-learning`: PLIV/IIVM, high-dimensional controls, flexible nuisance support.
- `33-survival-competing-risks`: survival outcomes or censoring with IV.
- `15-negative-controls-proximal`: falsification, proxy, and unmeasured-confounding probes around exclusion.
