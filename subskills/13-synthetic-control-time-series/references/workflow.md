...
| Many treated cohorts/policy rollout | DiD/event-study | Staggered adoption estimands | Activate `10-did-event-study` |
| Spillover-prone donors | Interference/spillover review | Donor contamination breaks counterfactual | Activate `14-interference-spillovers` |

## 4. Coordinate With Other Subskills

- `10-did-event-study`: staggered adoption, group-time ATT, event-study diagnostics, synthetic DiD overlap.
- `14-interference-spillovers`: donor contamination, geographic spillovers, market/network interference.
- `24-transportability-generalizability`: treated-unit specificity and target-population generalization.
- `23-dose-response-effects`: intervention intensity or partial adoption over time.
- `30-matching-weighting-balance`: donor weighting and balance logic where relevant.
- `32-double-machine-learning`: matrix completion/ML prediction support, if used as implementation support.
- `33-survival-competing-risks`: time-to-event/rate outcomes in aggregate panels.
