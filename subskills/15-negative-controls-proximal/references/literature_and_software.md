...
| [`pci2s`](https://arxiv.org/abs/2409.08924) | R/research | Regression-based proximal causal inference for right-censored time-to-event data | Useful when survival/censoring and proxies are central | Newer research implementation; coordinate with survival module |
...
- Need survival outcomes: coordinate with `33-survival-competing-risks`; consider proximal survival tools only if censoring and bridge assumptions are explicit.
- Need longitudinal treatment/confounding: coordinate with `09-longitudinal-gmethods`; simple point-treatment proximal templates are not enough.
