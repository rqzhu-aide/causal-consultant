...
| [`DynTxRegime`](https://cran-e.com/package/DynTxRegime) | R | Single-stage or dynamic optimal treatment regimes | Q-learning, weighted learning, value-search methods | Dynamic use belongs to `25-dynamic-treatment-policies`; interface is specialized |
...
- Need observational policy learning: prefer doubly robust/orthogonal approaches and activate `31-doubly-robust-estimation` or `32-double-machine-learning`.
- Need survival outcome: keep this target module active, but activate `33-survival-competing-risks` for censoring and estimand support.
