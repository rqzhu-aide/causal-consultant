...
If the user only wants "who benefits more", activate or read `20-heterogeneous-effects` first. If the user wants repeated decisions over time, activate `25-dynamic-treatment-policies`.
...
- Observational single-time exposure: possible with exchangeability, positivity, correct timing, and robust nuisance modeling; request `08-single-time-observational-exposure`, `30-matching-weighting-balance`, `31-doubly-robust-estimation`, or `32-double-machine-learning` support as needed.
...
- Longitudinal or repeated decisions: usually not this module; route to `09-longitudinal-gmethods` and `25-dynamic-treatment-policies`.
- Survival/time-to-event outcomes: keep the one-time decision target here, but request `33-survival-competing-risks` for censoring and time-scale support.
