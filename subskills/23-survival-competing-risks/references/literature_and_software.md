...
# Replace time, event, A, and covariates; pre-specify horizon and scale.
library(survival)

km <- survfit(Surv(time, event) ~ A, data = analysis_data)
cox <- coxph(Surv(time, event) ~ A + X1 + X2,
             data = analysis_data, robust = TRUE)
ph_diag <- cox.zph(cox)
```
