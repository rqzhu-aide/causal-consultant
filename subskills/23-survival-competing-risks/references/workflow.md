...
- `02-longitudinal-gmethods`: time-varying treatment, artificial censoring, clone-censor-weight, sequential g-formula, MSM, LMTP, or longitudinal TMLE.
- `10-heterogeneous-effects`: survival CATE/GATE/ITE, effect modification, causal survival forests, survival learner diagnostics.
- `11-point-treatment-rules`: one-time treatment rules using fixed-time risk, RMST, or survival CATE scores.
- `15-dynamic-treatment-policies`: sequential decision rules with time-to-event utility/value.
