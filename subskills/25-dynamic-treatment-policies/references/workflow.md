...
If there is only one decision point, use `21-point-treatment-rules`. If the user wants fixed sustained treatment effects, use `09-longitudinal-gmethods` first.
...
- Time-varying confounding affected by prior treatment: activate `09-longitudinal-gmethods`.
- Survival outcome: activate `33-survival-competing-risks`.
