...
- `08-single-time-observational-exposure`: primary design route for baseline observational treatment comparisons.
- `09-longitudinal-gmethods`: time-varying treatment/censoring weights and MSM support.
- `24-transportability-generalizability`: target-population or trial-to-target weighting.
- `31-doubly-robust-estimation`: AIPW/TMLE after propensity/balance support is reviewed.
- `32-double-machine-learning`: flexible propensity/nuisance learners and cross-fitting when high dimensional.
- `33-survival-competing-risks`: weighted survival or censoring-aware outcome support.
