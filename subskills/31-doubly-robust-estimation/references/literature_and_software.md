...
- Need longitudinal treatment/censoring: use `ltmle` or `lmtp` with `09-longitudinal-gmethods`.
- Need high-dimensional orthogonal ML: coordinate with `32-double-machine-learning`.
- Need heterogeneity or policy scores: DR scores can feed `20-heterogeneous-effects` or `21-point-treatment-rules`, but those modules define the target.
