...
- Need CATE/heterogeneity: use `grf` or EconML CausalForestDML with `20-heterogeneous-effects`.
- Need sparse linear controls: use post-double-selection via `hdm`.
- Need ML as a plugin inside AIPW/TMLE: coordinate with `31-doubly-robust-estimation`.
- Need IV with high-dimensional controls: use DML IV only with `12-instrumental-variables` support.
