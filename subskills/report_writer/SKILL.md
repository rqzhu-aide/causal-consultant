...
- `execution_records` and post-execution Return Gate state when analysis was run: completed unit, confirmed scope, claim boundary, intended tool lanes, dependency status, deviation status, source code path, analysis note path, external artifacts, embedded-table status, report asset plan, closeout status, queue reconciliation, report readiness, and next user choice.
...
- `artifact_index`: existing report plans, final HTML reports, code paths, tables, figures, and diagnostics.
...
Default to static HTML for final reports. Use `assets/final_report_template.html` for comprehensive reports unless main says the user explicitly requested a shorter memo-style HTML artifact. Private report plans may stay Markdown; `analysis_note_*.md` technical notes remain inputs, not final reports.

Draft only from recorded or inspected evidence, especially the Return Gate state and `analysis_note_*.md` when analysis was run. Do not add unsupported causal claims, numeric results, references, figures, diagnostics, or conclusions. Do not simplify away prior staged work: include the role map, method/fallback reasoning, selected spec, gatekeeper boundary, execution Return Gate, dependency notes, deviations, results, diagnostics, and next steps whenever those stages occurred.

Do not turn a substantive report into stacked tables. Each major evidence section should include short prose explaining why the section exists, how to read the evidence, what it means, and what it does not prove. Each table, figure, or inline causal-structure sketch needs a headline, interpretation, source/path or state reference, and limitation.
...
Before drafting a final or substantive report, check that `pending_user_intents` and worthwhile consultant alternatives in `team_synthesis.exploration_threads` or `method_alignments.method_ideas` have been resolved, declined, blocked, or explicitly parked for the report. If unresolved work remains, return `blocked` feedback with the one remaining item main should ask about next.

Also check the latest executed unit. If `closeout_status` is missing, `incomplete`, or `blocked`, or if `queue_reconciliation.report_ready` is `false`, return `blocked` feedback instead of drafting. Ask main to complete queue reconciliation or surface the next remaining item to the user.
...
Final HTML reports require QA for duplicate titles, missing wrapper or container, broken tables, broken local links, missing source script or artifact links, and visible claim boundary. Return the final HTML report path to main; do not ask about a later format step.

## Report-Ready QA

A substantive report is not ready if it lacks the consultation trace, role card, method/fallback choice, selected spec, validity boundary, required inline causal-structure sketch when causal/timing/adjustment logic matters, post-analysis gatekeeper review when analysis was run, resolved or explicitly parked pending user intents and worthwhile consultant alternatives, closed/parked/inactive discovery sidecar state when discovery was opened or requested, execution Return Gate state or `execution_records` item when analysis was run, `closeout_status: complete`, `queue_reconciliation.report_ready: true`, dependency/deviation notes, required report assets, citation/source notes, narrative section explanations, source script path when code supports results, final HTML report path, external artifact index when external files exist, or required owner review. Return `blocked`, `needs_assets`, or `artifact_revised` feedback to main rather than delivering a thin or script-authored report.
...
report_writer_feedback:
  action: no_action | plan_created | draft_created | artifact_revised | needs_assets | blocked
...
Ask main to record durable paths in `artifact_index` only. Ask main to record pending user-facing report choices in `team_synthesis.next_suggested_action` or `team_synthesis.open_questions`.
